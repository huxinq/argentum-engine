...
/** A label may consume only a game end or the declared completed evaluation boundary. */
internal fun completedEvaluationState(simulation: SimulationResult): GameState? = when (simulation) {
    is SimulationResult.GameEnded -> simulation.state
    is SimulationResult.Quiet -> simulation.state
    is SimulationResult.Illegal,
    is SimulationResult.NeedsDecision,
    is SimulationResult.StoppedAtLimit -> null
}
