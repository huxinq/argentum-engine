...
Immutability also makes canonical replay storage practical: record the complete initial state, then
lossless JSON patches between authoritative states, with periodic full-state checkpoints. Current
replays reconstruct directly without running engine or card logic. Historical input-only records
still rely on same-version determinism, pinned card definitions, position fingerprints, and an
archived spectator stream as a last resort — see [data-contracts.md](data-contracts.md) →
*Canonical replays*.
